...
function CampDetails () {
    ...
    useEffect (() => {
        ...
        fetch ('http://127.0.0.1:8000/camps/camp-rome/', {
            method: 'GET',
            headers: {
        ...
        // Fetch the data from the API (keep in mind that the current requesting address should be authorized in the API)
        fetch ('http://127.0.0.1:8000/camps/camp-rome/reviews/', {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            }
                ...
                <div className='mb-3'><Navbar/></div>
                    ...
                    <div className="position-relative text-center" >
                        ...
                        <div className='box bg-secondary bg-opacity-75'>
                            <p className="position-absolute  bottom-0 start-50 translate-middle-x  text-white fs-2">{campDetails.title}</p>
                        </div>
                    </div>
                    
                    {/* Main description of the camp */}
                    <div className='mt-3 mb-3 p-3 text-white h5 bg-secondary bg-opacity-50 radius'>{campDetails.main_body}</div>  

                    {/* Position description on the left and map on the right */}
                    <div className='d-flex mb-3'>
                        <div className= 'radius p-3 mb-2 bg-secondary bg-opacity-50 text-white h6 w-50'>{campDetails.position_body}</div>  
                        <div className='w-50 ms-2 radius'>
                            ...
                            {/* <MapContainer className="map" center={[campDetails.latitude, campDetails.longitude]} zoom={20} style={{height:"30vh"}}> 
                                <TileLayer attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> 
                                    ...
                                    <Marker position={[campDetails.latitude, campDetails.longitude]} icon={GetIcon(50)}> </Marker>
                            </MapContainer>       */}
                        </div>                           
                    </div> 
